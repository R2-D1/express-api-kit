...
exports.forgotPassword = async (request, response) => {
    ...
    try {
        user.save();
        await sendEmail(email, 'Reset password', "Hi there!", "It looks like you forgot your password, follow the link below and reset it.", "Reset password", "http://localhost:50946/reset-password/" + token);
        response.status(201).json({
            success: true,
        });
    } catch (error) {
...
exports.login = async (request, response) => {
    ...
    const isPasswordCorrect = await bcrypt.compare(password, user.password);
    const jwtToken = jwt.sign({email, role: user.role}, process.env.JWT_SECRET, {expiresIn: '432000s'})
